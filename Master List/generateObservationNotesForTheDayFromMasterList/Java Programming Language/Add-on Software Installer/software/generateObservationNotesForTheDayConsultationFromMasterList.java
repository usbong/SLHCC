...
'
' Output:
' 1) Auto-generated Observation Notes For the Day Consultation Report
' --> "Tab delimited" .txt file 
' --> Regardless of the name of the input file or input files, the output file will be "ObservationNotesForTheDayConsultationSummaryReportOutput.txt".
' --> The Report includes: 
' --> a) NON-HMO (net)
...
'   java generateObservationNotesForTheDayConsultationFromMasterList input201801.txt
' 
' where: "input201801.txt" is the name of the file.
' 
*/ 

public class generateObservationNotesForTheDayConsultationFromMasterList {	
	...
	private static final int INPUT_CLASS_COLUMN = 8; //HMO and NON-HMO Classifcations
	...
	//added by Mike, 20190217
	//We need to put an offset due to certain columns, e.g. INPUT_CLASS_COLUMN, in the PT Treatment workbook being located one (1) column to the right in the Consultation workbook
...
*/
	...
	{			
		...
		PrintWriter writer = new PrintWriter("output/ObservationNotesForTheDaySummaryConsultationReportOutput.txt", "UTF-8");			
...
*/			
			if (inputFilename.toLowerCase().contains("consultation")) {
				isConsultation=true;
				inputOffset=INPUT_CONSULTATION_OFFSET;
			}
			else {
...
//				System.out.println(">> s: "+s);
				...
				if ((inputColumns[INPUT_CLASS_COLUMN+inputOffset].contains("HMO")) ||
					(inputColumns[INPUT_CLASS_COLUMN+inputOffset].contains("SLR"))) {
					continue;
				}
				
				//phase/part/component 1
				int transactionORNumber = Integer.parseInt(inputColumns[INPUT_OR_NUMBER_COLUMN+inputOffset]);
				if (!transactionsContainer.containsKey(transactionORNumber)) {
					...
					transactionsValuesArray[OUTPUT_DATE_COLUMN] = inputColumns[INPUT_DATE_COLUMN]; //this does not require an inputOffset
					transactionsValuesArray[OUTPUT_PATIENT_NAME_COLUMN] = inputColumns[INPUT_PATIENT_NAME_COLUMN]; //this does not require an inputOffset
					transactionsValuesArray[OUTPUT_PAYMENT_CLASSIFICATION_COLUMN] = inputColumns[INPUT_CLASS_COLUMN+inputOffset];	
...
*/		
		...
		writer.print("Notes (Consultation):\n");
		int notesCount=1;
